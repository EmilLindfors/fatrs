...
impl TransactionEntry {
    ...
    /// Calculate CRC32 checksum of entry data
    pub fn calculate_crc32(&self) -> u32 {
        let mut crc = 0xFFFF_FFFFu32;

        // Hash magic, version, type, state
        crc = Self::update_crc32(crc, &self.magic.to_le_bytes());
        crc = Self::update_crc32(crc, &self.version.to_le_bytes());
        crc = Self::update_crc32(crc, &[self.tx_type as u8]);
        crc = Self::update_crc32(crc, &[self.state as u8]);
        crc = Self::update_crc32(crc, &self.sequence.to_le_bytes());
        crc = Self::update_crc32(crc, &self.timestamp.to_le_bytes());
        crc = Self::update_crc32(crc, &self.sector_count.to_le_bytes());

        // Hash affected sectors
        for &sector in &self.affected_sectors[..self.sector_count as usize] {
            crc = Self::update_crc32(crc, &sector.to_le_bytes());
        }

        // Hash backup data
        crc = Self::update_crc32(crc, &self.backup_data);

        !crc
    }

    /// Update CRC32 with data bytes
